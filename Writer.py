...
def make_file(output, sheet_name, path):
    int_des = output['display interface description']
    dis_int = output['display interface']
    dis_int_eth = output['display interface eth-trunk']
    int_lic = output['display license verbose']
    lic_res = output['display license resource usage']
    pic_status = output['display device pic-status']
    opt_module = output['display optical-module brief']
    port_basic = output['display license resource usage port-basic all']
    dis_version = output['display version']
    dis_elable_brief = output['display elabel brief']
...
class Writer():
    def __init__(self, output, device_name, path):
        self.t = threading.Thread(args=(device_name, output, path), target=make_file(output, device_name, path))
        self.t.start()
