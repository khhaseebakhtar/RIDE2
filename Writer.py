...
def make_file1(output, sheet_name, path, vendor, exe):
        ...
        wider_cells = [('T','AA','AC','AJ','U','AJ',10),
                       ('A','G','J','AA','AC','AI','AL','AK',20),
                       ('P','W','AM','AM',30),
                       ('D','S',50)]
        ...
        if 'huawei' in vendor:
            optics_colmns = {'Z': 'port', 'AA': 'status', 'AB': 'duplex', 'AC': 'type', 'AD': 'wl', 'AE': 'rxpw', 'AF': 'txpw',
                    'AG': 'mode'}
        ...
        for cell, value in headers.items():
            ...
            elif cell.startswith(('M1', 'N1', 'O1', 'P1','T20','U20','V20','W20','X20', 'Y20')):
        ...
        write_data(trunks, 2,{'M': 'trunk_number','N':'trunk_state','O': 'no_of_links','P': 'member_interfaces'},vendor)
...
class Writer():
    def __init__(self, output, device_name, path, vendor,exe):
        self.t = threading.Thread(args=(device_name, output, path,vendor,exe), target=make_file1(output, device_name, path,vendor,exe))
        self.t.start()
