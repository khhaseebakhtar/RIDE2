...
class SessionManager(QObject):
    ...
    def convert_to_writable_formate(self, data):
        for command in data:
            ...
            if 'show interfaces descriptions | display json | no-more' in keys and command[0][
                'show interfaces descriptions | display json | no-more'] != "":
                logical_interfaces: list = \
                    command[0]['show interfaces descriptions | display json | no-more']['interface-information'][0][
                        'logical-interface']
                ...
                def process_interfaces(interfaces_list):
                    for each_interface in interfaces_list:
                        name = self.if_key_found(each_interface, key='name')
                        phy_status = self.if_key_found(each_interface, key='admin-status')
                        description = self.if_key_found(each_interface, key='description')
                        opr_status = self.if_key_found(each_interface, key='oper_status')
                ...
                process_interfaces(logical_interfaces)
                process_interfaces(physical_interfaces)
